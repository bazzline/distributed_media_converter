...
function create_file_list ()
{
    ...
    #we simple don't want to deal with files like >>./foo/bar.mp4<<
    #it is way easier if we only have full qualified file paths like /baz/foo/bar.mp4
    ...
    #find "${WORKING_DIRECTORY}" -type f -exec file {} \; | grep -o -P '^.+: \w+ video' > "${PATH_TO_THE_FILE_LIST}"
